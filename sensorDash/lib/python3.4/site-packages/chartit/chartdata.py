...
class DataPool(object):
    ...
    def _group_terms_by_query(self, sort_by_term=None, *addl_grp_terms):
        ...
        tk_td = (tk, td)
        sort_grp_fn = lambda tk_td: tuple(chain(str(td['source'].query),
        ...
        if sort_by_term is not None:
            sort_by_fn = lambda tk_td: -1 * (abs(td[sort_by_term]))
        else:
            sort_by_fn = None
...
class PivotDataPool(DataPool):
    ...
    def _generate_vqs(self):
        ...
        for tk_td_tuples in self.query_groups:
            ...
            ann_terms = collections.OrderedDict(
                (k, d['func']) for k, d in tk_td_tuples)
    ...
    def _get_data(self):
        ...
        if self.pareto_term:
            ...
            if self.mapf is None:
                ...
                if self.mts:
                    combined = sorted(
                        zip(self.cv, self.cv_raw), key=self.sortf)
